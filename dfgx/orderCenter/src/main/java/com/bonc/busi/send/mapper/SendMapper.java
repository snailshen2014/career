...
public interface SendMapper {
	 ...
	 * @return
	 */
	@Select("SELECT count(1) FROM PLT_ORDER_INFO_SMS WHERE TENANT_ID=#{tenantId} AND CHANNEL_ID=#{channelId} AND ACTIVITY_SEQ_ID=#{activitySeqId} "
	 ...
	 * @param reqMap
	 */
	@Update("UPDATE PLT_ACTIVITY_CHANNEL_STATUS SET STATUS=#{status} WHERE  TENANT_ID=#{tenantId} and ACTIVITY_SEQ_ID=#{activitySqlId} ") 
