...
public class DateUtil {
	...
	public static class DateFomart{
		...
		public static String EN_MONTH="yyyy-MM";
...
//            c.add(Calendar.MONTH, -1);//今天的时间月份-1支持1月的上月
    ...
    public static String getLastStatisticDate(){
    	...
    	return new SimpleDateFormat(DateFomart.EN_DATETIME).format(ncalendar.getTimeInMillis());
    };
...
//    	ncalendar.set(Calendar.MONTH, 8);
    ...
    public static List<String> getMonthList(String start,String end,String format) throws ParseException{
    	List<String> monthList = new ArrayList<String>();
    	SimpleDateFormat monthf=new SimpleDateFormat(DateFomart.EN_DATETIME);
    	end = null==end?CurrentDate.currentDateFomart(format):end;
    	int count = countMonths(start,end,format);
    	monthList.add(start);
    	Calendar calendar=Calendar.getInstance();
    	calendar.setTime(monthf.parse(start));
    	while (count-->0) {
    		calendar.set(Calendar.MONTH,calendar.get(Calendar.MONTH)+1);
    		monthList.add(FomartDate.toFormat(calendar.getTime(), DateFomart.EN_MONTH));
		}
    	monthList.add(end);
...
//    	System.out.println(getCurMonth());
    	List<String> months = getMonthList("201611111111",null, DateFomart.EN_MINUTE);
