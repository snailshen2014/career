...
public interface StatisticActivityMapper{
	...
	//1.1
	@Select("SELECT MIN(REC_ID) minId,MAX(REC_ID) maxId,TENANT_ID tenantId FROM PLT_ORDER_CONTACT_TEMP WHERE TENANT_ID =#{tenantId}")
	HashMap<String, Object> getOrders(String curTenantId);

	//1.2
	@Select("SELECT COUNT(1) count,ACTIVITY_SEQ_ID,CHANNEL_ID,TENANT_ID tenantId FROM PLT_ORDER_CONTACT_TEMP WHERE TENANT_ID=#{tenantId} AND REC_ID >= ${minId} AND REC_ID <= ${maxId}  GROUP BY ACTIVITY_SEQ_ID,CHANNEL_ID")
	List<HashMap<String, Object>> getListActivity(HashMap<String, Object> orders);

	//1.3
	@Update("UPDATE PLT_ORDER_STATISTIC_SEND SET SEND_SUC_NUM=SEND_SUC_NUM+${count} WHERE TENANT_ID=#{tenantId} AND ACTIVITY_SEQ_ID=#{ACTIVITY_SEQ_ID}  AND CHANNEL_ID=#{CHANNEL_ID}")
	void updateStatisSend(HashMap<String, Object> curActivity);

	//1.4
	@Update(" DELETE FROM PLT_ORDER_CONTACT_TEMP WHERE TENANT_ID=#{tenantId} AND REC_ID >= ${minId} AND REC_ID <= ${maxId}")
	void deleteOrders(HashMap<String, Object> orders);

	@Update("UPDATE PLT_ORDER_INFO SET WENDING_FLAG=NULL WHERE TENANT_ID=#{tenantId} AND ACTIVITY_SEQ_ID=#{activitySeqId} AND CHANNEL_ID=#{channelId} AND WENDING_FLAG=''")
	void updateEmptyLoginId(HashMap<String, Object> req);
