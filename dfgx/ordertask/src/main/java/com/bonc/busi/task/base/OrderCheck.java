...
public class OrderCheck extends ParallelFunc {
	...
	public Object get() {
		...
		while (1 > 0) {
			if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO") || m_strTableName == null) {
				listOrderCheckInfo = BaseMapperIns.getOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_POPWIN")) {
				listOrderCheckInfo = BaseMapperIns.getPopwinOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_WEIXIN")) {
				listOrderCheckInfo = BaseMapperIns.getWeiXinOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_ONE")) {
				listOrderCheckInfo = BaseMapperIns.getOneOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SMS_HIS_" + OrderInsertTime)) {
				listOrderCheckInfo = BaseMapperIns.getSmsHisOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId, "PLT_ORDER_INFO_SMS_HIS_" + OrderInsertTime);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SMS_HIS_" + OrderlastTime)) {
				listOrderCheckInfo = BaseMapperIns.getSmsHisOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId, "PLT_ORDER_INFO_SMS_HIS_" + OrderlastTime);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SCENEMARKET")) {
				listOrderCheckInfo = BaseMapperIns.getScenemarketOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_REMAIN")) {
				listOrderCheckInfo = BaseMapperIns.getRemainOrderListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_HIS")) {
				listOrderCheckInfo = BaseMapperIns.getOrderHisListForActivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_CALL")) {
				listOrderCheckInfo = BaseMapperIns.getCallOrderForAvtivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SMALLWO")) {
				listOrderCheckInfo = BaseMapperIns.getSmallwoOrderForAvtivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_PLAY")) {
				listOrderCheckInfo = BaseMapperIns.getPlayOrderForAvtivity(curTenantId, curActivitySeqId,
						curOrderRecId);
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_NEWWSC")) {
				listOrderCheckInfo = BaseMapperIns.getNewWSCForAvtivity(curTenantId, curActivitySeqId, curOrderRecId);
			} else {
			...
			if (listOrderCheckInfo == null || listOrderCheckInfo.size() == 0) { // ---
				...
				// --- 调用统计程序统计 ---
				// if(m_iTotalNum > 0)
				// StatisticServiceIns.statisticBench(curTenantId,
				// String.valueOf(curActivitySeqId));
				// log.info("结束调用统计程序");
				// --------------------------

				// --- 判断下一个活动ID是否存在 ---
				if (getValidTenantIdAndAvitivtySeqId() == false) { // ---
																	// 无有效数据,退出
																	// ---
					return null;
				} else {
		...
		log.info("租户ID:" + curTenantId + " 活动序列号:" + curActivitySeqId + "  起始工单号:" + curOrderRecId + "  数据条数: "
				+ listOrderCheckInfo.size() + "当前总条数:" + m_iTotalNum);
	...
	public int handle(Object data) {
		...
		String SucessTypeSql = getSqlForSucessType(TenantId, ActivitySeqId, listOrderCheckInfo);
		if (SucessTypeSql == null)
			return -1;
		...
		// 判断括号数量相不相同
		/*
		 * String sbString = sb.toString(); int x = 0; int y = 0; for(int j = 0
		 * ;j < sbString.length();j++){ if(sbString.substring(j,
		 * j+1).equals("(")||sbString.substring(j, j+1).equals("[")){ x++; }
		 * if(sbString.substring(j, j+1).equals(")")||sbString.substring(j,
		 * j+1).equals("]")){ y++; } }
		 */

		/*
		 * sb.append(" AND USER_ID IN ("); int i = 0; for(i = 0; i <
		 * listOrderCheckInfo.size();++i){ if(i > 0) sb.append(",");
		 * sb.append("'"); sb.append(listOrderCheckInfo.get(i).getUserId());
		 * sb.append("'"); } sb.append(")");
		 */

		/*
		 * if(x==y){ log.info("左右括号数量相同均为"+x); }else { sb.append(")");
		...
		if (m_strTableName == m_tableList.get(0).get("tableName")) {
			for (HashMap<String, Object> tableName : m_tableList) {
				sbb.setLength(0);
				sbb.append("UPDATE " + tableName.get("tableName")
						+ " SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = " + curDateId
						+ " ");
			...
			if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO") || m_strTableName == null) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_POPWIN")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_POPWIN SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_WEIXIN")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_WEIXIN SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_ONE")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_ONE SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SMS_HIS_" + OrderInsertTime)) {
				sbb.append("UPDATE PLT_ORDER_INFO_SMS_HIS_" + OrderInsertTime
						+ " SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = " + curDateId
						+ " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SMS_HIS_" + OrderlastTime)) {
				sbb.append("UPDATE PLT_ORDER_INFO_SMS_HIS_" + OrderlastTime
						+ " SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = " + curDateId
						+ " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SCENEMARKET")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_SCENEMARKET SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_REMAIN")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_REMAIN SET  CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_HIS")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_HIS SET CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_CALL")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_CALL SET CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_SMALLWO")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_SMALLWO SET CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_PLAY")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_PLAY SET CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			} else if (m_strTableName.equalsIgnoreCase("PLT_ORDER_INFO_NEWWSC")) {
				sbb.append(
						"UPDATE PLT_ORDER_INFO_NEWWSC SET CHANNEL_STATUS = '3' ,LAST_UPDATE_TIME = now() ,AGREEMENT_EXPIRE_TIME = "
								+ curDateId + " ");
			}
			sbb.append(" WHERE TENANT_ID='");
			sbb.append(TenantId);
	...
	private boolean getValidTenantIdAndAvitivtySeqId() {
		...
		if (curTenantId == null) { // --- 第一次提取 ---
			for (i = 0; i < listTenantInfo.size(); ++i) {
				String TENANT_ID = (String) listTenantInfo.get(i).get("TENANT_ID"); // ---
																					// 得到租户编号
																					// ---
	...
	private String getSqlForSucessTypeOld(String tenant_id, int activity_seq_id) {
		// --- 得到活动成功信息 ---
		...
		if (bProduct) {
			...
			log.info("精准营销：");
			// log.info("工单检查sql="+sbProduct.toString()+"),
			// 精准营销时，成功类型="+successType);
		...
		// IS_SIM = '0' ");
		// if(bProduct){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE CHR_18 = '0' ");
		...
		// if(bProduct){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE TE_N_04='4' ");
		// sbProduct.append(" AND DATE_ID = '");
		// sbProduct.append(curDateId);
		// sbProduct.append("' ");
		// if(strAddSql != null){
		...
		// }
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("3")){ // ---
		// 办理流量包 ---
		// if(bProduct){
		// if(strAddSql != null){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE ");
		// sbProduct.append(strAddSql);
		// sbProduct.append(" AND DATE_ID = '");
		// sbProduct.append(curDateId);
		// sbProduct.append("' ");
		...
		// }
		// else{
		// sb.append(" SELECT USER_ID FROM CLJY_ALL_ACCEPTED_LIST WHERE
		// PROD_TYPE ='03' ");
		// sb.append(" AND ACCEPTED_DATE > '");
		// sb.append(OrderCreateTime);
		// sb.append("' ");
		// if(strAddSql != null){
		// sb.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE ");
		// sb.append(strAddSql);
		// sb.append(" AND DATE_ID = '");
		// sb.append(curDateId);
		...
		// //return sbProduct.toString();
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("4")){ // ---
		// 办理续约 ---
		// if(bProduct){
		// if(strAddSql != null){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		...
		// //return sbProduct.toString();
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("5")){ // ---
		// 换套餐 ---
		// if(bProduct){
		// if(strAddSql != null){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		...
		// //return sbProduct.toString();
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("6")){ // ---
		// 实名登记 ---
		// //sb.append(" SELECT USER_ID FROM UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE
		// BA_S_04='1' ");
		// if(bProduct){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE BA_S_04='1' ");
		// sbProduct.append(" AND DATE_ID = '");
		// sbProduct.append(curDateId);
		// sbProduct.append("' ");
		// if(strAddSql != null){
		...
		// }
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("7")){ // ---
		// 宽带续约 ---
		// //sb.append(" SELECT USER_ID FROM UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE
		// KD_IS_KDDQX='1' ");
		// if(bProduct){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE KD_IS_KDDQXQ='1' ");
		// sbProduct.append(" AND DATE_ID = '");
		// sbProduct.append(curDateId);
		// sbProduct.append("' ");
		// if(strAddSql != null){
		...
		// }
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("8")){ // ---
		// 办理副卡 ---
		// //sb.append(" SELECT USER_ID FROM UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE
		// CHR_17='1' ");
		// if(bProduct){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE CHR_17='1' ");
		// sbProduct.append(" AND DATE_ID = '");
		// sbProduct.append(curDateId);
		// sbProduct.append("' ");
		// if(strAddSql != null){
		...
		// }
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("9")){ // ---
		// 4G登网 ---
		// //sb.append(" SELECT USER_ID FROM UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE
		// TYPE_HYBL_CUST_TYPE='40AAAAAA' ");
		// if(bProduct){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		// UNICOM_D_MB_DS_ALL_LABEL_INFO WHERE TYPE_HYBL_CUST_TYPE='40AAAAAA'
		// ");
		// sbProduct.append(" AND DATE_ID = '");
		// sbProduct.append(curDateId);
		// sbProduct.append("' ");
		...
		// }
		// }
		// else if(ActivitySucessInfoIns.getSucessType().equals("10")){ // ---
		// 承诺抵消 ---
		// if(bProduct){
		// if(strAddSql != null){
		// sbProduct.append(" AND USER_ID IN ( SELECT USER_ID FROM
		...
		// String strTmp =
		// ActivitySucessInfoIns.getSUCCESS_TYPE_CON_SQL().toUpperCase();
