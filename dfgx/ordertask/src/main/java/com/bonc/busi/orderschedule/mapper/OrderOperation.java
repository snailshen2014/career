...
public class OrderOperation {
	 ...
	 * @return
	 */
	public String genOrderRec(String sql) {
		return sql;
	}
	...
	public String InsertChannelFrontline(FrontlineChannelPo frontline) {
		...
		if (filter_list != null) {
			...
			if(jsonString.length()>4999){
				jsonString = jsonString.substring(0, 4500);
			}
	...
	public String InsertChannelTel(TelePhoneChannelPo tel){
		String s = "'";
		BEGIN();
		INSERT_INTO(CHANNEL_DETAIL);
		VALUES("CHANN_ID", s + tel.getChannelId() + s);
		VALUES("CONTENT", s + tel.getTelephoneHuashuContent() + s);
		VALUES("TENANT_ID", s + tel.getTenantId() + s);
		VALUES("ACTIVITY_ID", s + tel.getActivityId() + s);
		VALUES("FILTER_CON", s + tel.getFilterCondition() + s);
		VALUES("ACTIVITY_SEQ_ID", s + tel.getACTIVITY_SEQ_ID() + s);
		String sql_con = tel.getFilterConditionSql();
		if (sql_con != null) {
			if (sql_con.indexOf("'") != -1)
				sql_con = sql_con.replace("'", "\\'");
			VALUES("FILTER_SQL", s + sql_con + s);
		...
		String sql = SQL();
		logger.info("InsertChannelTel record sql:" + sql);
		return sql;
	}
	...
	public String InsertSuccessStandardPo(SuccessStandardPo success) {
		...
		VALUES("ISHAVERENEWPRODUCT", s + success.getIsHaveRenewProduct() + s);
		String sql = SQL();
	...
	// 移入历史工单（除短信）
	public String insertOrderInfoHis(OrderAndOrderSMS in) {
	...
	// 移入历史工单（短信）
	public String insertOrderInfoSMSHis(OrderAndOrderSMS in) {
		String s = "'";
		BEGIN();
		INSERT_INTO("PLT_ORDER_INFO_SMS_HIS");
		VALUES("REC_ID", s + in.getREC_ID() + s);
		VALUES("ACTIVITY_SEQ_ID", s + in.getACTIVITY_SEQ_ID() + s);
		VALUES("BATCH_ID", s + in.getBATCH_ID() + s);
		VALUES("CHANNEL_ID", s + in.getCHANNEL_ID() + s);
	...
	public String InsertActivityProcessLog(ActivityProcessLog log) {
		...
		VALUES("BEGIN_DATE", s + log.getBEGIN_DATE() + s);
	...
	public String UpdateActivityProcessLog(ActivityProcessLog log) {
		...
		SET("CHANNEL_ORDER_NUM=" + s + log.getCHANNEL_ORDER_NUM() + s);
		SET("END_DATE=" + s + log.getEND_DATE() + s);
	 ...
	 * @return
	 */
	public String updateOrderInfoStatusFilterBlack(OrderAndOrderSMS order) {
	 ...
	 * @return
	 */
	public String updateOrderInfoStatusFilterWhite(OrderAndOrderSMS order) {
		String table = this.getTableByChannelId(order.getCHANNEL_ID());
		String s = "'";
		StringBuilder whereBuilder = new StringBuilder();
		whereBuilder.append("PHONE_NUMBER=" + s + order.getPHONE_NUMBER() + s);
	 ...
	 * @return
	 */
	public String updateOrderSmsStatusFilterBlack(OrderAndOrderSMS order) {
		String s = "'";
		StringBuilder whereBuilder = new StringBuilder();
		whereBuilder.append("PHONE_NUMBER=" + s + order.getPHONE_NUMBER() + s);
		whereBuilder.append("AND  TENANT_ID= " + s + order.getTENANT_ID() + s);
	 ...
	 * @return
	 */
	public String updateOrderSmsStatusFilterWhite(OrderAndOrderSMS order) {
		String s = "'";
		StringBuilder whereBuilder = new StringBuilder();
		whereBuilder.append("PHONE_NUMBER=" + s + order.getPHONE_NUMBER() + s);
		whereBuilder.append("AND  TENANT_ID= " + s + order.getTENANT_ID() + s);
	...
	public String getInvalidActivitySeqId(String tenantId) {
		...
		BEGIN();
		SELECT("REC_ID, ACTIVITY_ID,ORDER_END_DATE");
		FROM(PLT_ACTIVITY_INFO);
		WHERE(whereBuilder.toString());
		return SQL();
	}
	...
	// 删除工单表
	public String deleteOrderByRecId(OrderAndOrderSMS order) {
		String table = this.getTableByChannelId(order.getCHANNEL_ID());
		String s = "'";
		StringBuilder wherebBuilder = new StringBuilder();
	...
	// 删除短信工单表
	public String deleteOrderSmsByRecId(OrderAndOrderSMS order) {
		String s = "'";
		StringBuilder wherebBuilder = new StringBuilder();
		wherebBuilder.append(" REC_ID=" + order.getREC_ID());
		wherebBuilder.append(" AND TENANT_ID= " + s + order.getTENANT_ID() + s);
		BEGIN();
		DELETE_FROM("PLT_ORDER_INFO_SMS");
		WHERE(wherebBuilder.toString());
		return SQL();
	}
	...
	public String getTableByChannelId(String channelId) {
		...
		else if (channelId.equals("14"))
			return "PLT_ORDER_INFO_CALL";
		else if(channelId.equals("13")) {
			return "PLT_ORDER_INFO_SMALLWO";
		}
		else if(channelId.equals("19")) {
			return "PLT_ORDER_INFO_PLAY";
		}
		else if(channelId.equals("20")) {
			return "PLT_ORDER_INFO_NEWWSC";
		}
		else
			return "PLT_ORDER_INFO";
	}
	 ...
	 */
	public String getWhitBlackUserList(int begin,int end) {
		BEGIN();
		SELECT("USER_ID,USER_PHONE,FILTE_TYPE");
		FROM("(select rownum rn,USER_ID,USER_PHONE,FILTE_TYPE from CLYX_ACTIVITY_FILTE_USERS) t");
	...
	public String getWhitBlackUserSize() {
		BEGIN();
		SELECT("count(*)");
		FROM("CLYX_ACTIVITY_FILTE_USERS");
		return SQL();
	...
	public String insertRemindOrder(Map<String, Object> param) {
		...
		BEGIN();
		INSERT_INTO("PLT_ORDER_INFO_REMAIN" + whereBuilder);
...
//		}
	...
	public String updateCoveredRuleOrder(Map<String, Object> param) {
		...
		whereBuilder.append(" AND a.TENANT_ID= '" + param.get("TENANT_ID") + "' ");
		whereBuilder.append(" AND a.CHANNEL_ID='" + param.get("CHANNEL_ID") + "'");
		whereBuilder.append(" AND a.ORDER_STATUS='5' AND a.CONTACT_CODE='0'");//20171225,经过确认,由CHANNEL_STATUS修改为CONTACT_CODE
...
//		}
	...
	public String insertOrderToHis(ParamMap param) {
		...
		String hisTable = table + "_HIS";
		SimpleDateFormat sdfYm = new SimpleDateFormat("yyyy-MM");
		String currentMoth = sdfYm.format(new Date()).substring(5, 7);
		hisTable += "_";
		hisTable += currentMoth;
		StringBuilder whereBuilder = new StringBuilder();
		whereBuilder.append(" select * FROM " + table + " where ");
