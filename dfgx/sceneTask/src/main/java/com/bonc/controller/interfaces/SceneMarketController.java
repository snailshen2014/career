...
//		fields.add("contactDateEnd");
	...
	@RequestMapping(value = "/queryHandleNum ")
	public Object queryHandleNum(@RequestBody HashMap<String, Object> req) {
		log.info("场景成功处理数数查询请求参数——>" + JSON.toJSONString(req));
		JsonResult JsonResultIns = new JsonResult();
		if (null == req) {
			JsonResultIns.setCode("1");
			JsonResultIns.setMessage("场景营销请求参数不能为空！");
		...
		try{
			long start = System.currentTimeMillis();
			List<HashMap<String, Object>> result = sceneService.queryHandleNum(req);
			long end = System.currentTimeMillis();
			log.info("场景营销成功工单数查询接口总耗时——>>>>"+(end-start)/1000.0+"s");
			return JSON.toJSONString(result);
		}catch(BoncExpection e){
