...
public class BusiTools {
	 ...
	 * 从全局狀態表中得到参数
	 */
	public		String		getValueFromStatus(String strKey){
		return TaskBaseMapperIns.getValueFromSysStatusCfg(strKey);
	}
	/*
	 * 设置全局变量，同时更新数据库
	 */
	...
	/**
	 * set program running status,replace setValueToGlobal
	 * setValueToGlobal no thread safe ,this by CFG_VALUE=0 and update affect rows avoid it.
	 * @param strKey
	 * @param strValue
	 * @return
	 * @author yanjunshen 
	 */
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	public	 boolean	setProgramRunningStatus(String strKey,String strValue){
		return TaskBaseMapperIns.updateProgramRunningStatus(strKey, strValue) == 1 ? true: false;
	}
	/*
	 * 得到唯一的序列号
	 */
	 ...
	 * 在行云上执行DDL语句
	 */
	public		JsonResult		execDdlOnXcloud(String	sqlDdl){
		...
		try{
			...
			connection = DriverManager.getConnection(getValueFromGlobal("DS.XCLOUD.URL") , 
	 ...
	 * @return
	 */
	public Map<String, Object> getLastExeActivity(final  String tenantId) {
		return jdbcTemplate.queryForMap("SELECT ACTIVITY_ID,ACTIVITY_SEQ_ID,BUSI_CODE  FROM PLT_ACTIVITY_EXECUTE_LOG" +
				" WHERE TENANT_ID = ? ORDER BY REC_ID desc LIMIT 1",new Object[]{tenantId});
	}

	public int deleteActivityInfo(final Integer activity_seq_id, final String tenantId) {
		return jdbcTemplate.update("DELETE FROM PLT_ACTIVITY_INFO WHERE  T" +
				"ENANT_ID = ? AND REC_ID = ?",new Object[]{tenantId,activity_seq_id});
	}
}
