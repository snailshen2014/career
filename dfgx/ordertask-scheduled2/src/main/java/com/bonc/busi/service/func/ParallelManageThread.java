...
public class ParallelManageThread extends Thread{
	// --- 定义日志变量 ---
	private final static Logger log= LoggerFactory.getLogger(ParallelManageThread.class);
	// --- 定义承载的基础实例变量 ---
	...
	// --- 构造函数 (基础实例必须由外部传入)---
	public	ParallelManageThread(ParallelFunc ParallelFuncIns){
		this.ParallelFuncIns = ParallelFuncIns;
	}
	// --- 构造函数 （设置线程数据）---
	public	ParallelManageThread(ParallelFunc ParallelFuncIns,int ThreadNum){
		this.ParallelFuncIns = ParallelFuncIns;
		this.iThreadNum = ThreadNum;
	}
	// --- 构造函数 （设置线程数，最大超时时间 ）---
	public	ParallelManageThread(ParallelFunc ParallelFuncIns,int ThreadNum,int MaxWaitSeconds){
		this.ParallelFuncIns = ParallelFuncIns;
		this.iThreadNum = ThreadNum;
