...
public class BusiTools {
    ...
    public int getSequence(String SequenceName) {
        ...
        Map<String, Object> mapResult = jdbcTemplate
                .queryForMap("SELECT INIT_VALUE,CUR_VALUE,STEP,MAX_VALUE FROM SYS_SEQUENCE "
        ...
        if (iCur_Value == -1 || (iCur_Value >= iMax_Value && iMax_Value != -1)) {
            // --- 更新初始值 ---
            jdbcTemplate.execute("UPDATE SYS_SEQUENCE SET CUR_VALUE = INIT_VALUE WHERE " + " SEQUENCE_NAME = '"
                    + SequenceName + "'");
            return iInit_Value;
        }
        int sequenceId = -1;
