...
import com.bonc.busi.outer.mapper.ActivityDetailMapper;
import com.bonc.busi.outer.service.ActivityDetailService;
...
//	    }
	...
	@Override
	public String channelOrderCountV2(HashMap<String, Object> parameter) {
		String tenantId = (String) parameter.get("tenantId");
		String activityIds = (String) parameter.get("activityIdList");
		HashMap<String, Object> resultMap = new HashMap<String, Object>();
		if(StringUtils.isBlank(tenantId) || StringUtils.isBlank(activityIds)){
			setCodeAndMsg("0001", "查询失败,参数不完整", resultMap);
			resultMap.put("tenantId", tenantId!=null?tenantId:"0000");
			return JSON.toJSONString(resultMap);
		}
		//参数字段转化
	    parameter.put("activityIdList", Arrays.asList(String.valueOf(parameter.get("activityIdList")).split(",")));
	    ...
	    List<HashMap<String, Object>> activityChannelOrderCountList = new  ArrayList<HashMap<String, Object>>();
        for(String activityId:(List<String>)parameter.get("activityIdList")){
        	//根据活动Id、LAST_UPDATE_TIME降序查询活动表PLT_ACTIVITY_INFO ACTIVITY_STATUS=1的,LIMIT 1 ,就是最新批次的
        	Integer activitySeqId = baseMapper.queryRecentSeqId(tenantId,activityId);
        	if(activitySeqId !=null){
        	log.info("======channelOrderCountV2,活动："+ activityId +", process_log表里最新的批次:"+activitySeqId);
	    	List<HashMap<String, Object>> channelOrderCount = activityDetailMapper.getChannelOrderCountByActivityId(
	    	activityId,String.valueOf(parameter.get("tenantId")),activitySeqId);
	    	Boolean validateResultList = validateResultList(channelOrderCount, resultMap);
	  		if(validateResultList){
	  			HashMap<String, Object> activityChannelOrderCount = new HashMap<String, Object>();
	  			activityChannelOrderCount.put("activityId",activityId);
