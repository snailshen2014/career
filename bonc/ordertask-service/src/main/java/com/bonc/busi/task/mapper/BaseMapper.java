...
import com.bonc.busi.task.base.bo.ActivitySucessInfo;
import com.bonc.busi.task.base.bo.OrderCheckInfo;
import com.bonc.busi.task.base.bo.PltCommonLog;
...
public interface BaseMapper {
	   ...
	   // --- 更新序列表 ---
	    ...
	    //查询plt_activity_process_log表里指定的活动的最大的rec_id
	    @Select("SELECT  MAX(REC_ID) FROM PLT_ACTIVITY_PROCESS_LOG WHERE TENANT_ID=#{tenantId} AND ACTIVITY_ID=#{activityId}")
	    public int queryMaxRecId(@Param("tenantId")String tenantId, @Param("activityId")String activityId);
		
	    //查询活动表里指定的活动的最新的批次
	    @Select("SELECT REC_ID from PLT_ACTIVITY_INFO WHERE "
	    		+ " TENANT_ID=#{tenantId} AND ACTIVITY_ID=#{activityId} AND ACTIVITY_STATUS='1'  ORDER BY LAST_ORDER_CREATE_TIME DESC LIMIT 1")
	    public Integer queryRecentSeqId(@Param("tenantId")String tenantId, @Param("activityId")String activityId);
