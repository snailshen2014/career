...
public interface BaseMapper {
	   ...
	   // --- 更新序列表 ---
	    ...
	    // --- 更新SYS_COMMON_CFG ---
	    @Update("update SYS_COMMON_CFG SET CFG_VALUE = #{strValue} "
	            + " where CFG_KEY=#{strKey} and CFG_VALUE='0'")
	    public int updateProgramRunningStatus(@Param("strKey") String strKey,@Param("strValue") String strValue);
	    // --- 插入通用日志表 ---
	    ...
	    // --- 查询系统状态表 --
	    @Select("SELECT CFG_VALUE FROM SYS_RUNNING_CFG WHERE CFG_KEY = #{strCfgKey}")
	  	public String getValueFromSysStatusCfg(@Param("strCfgKey") String strCfgKey);
	    // --- 更新系统变量 ---
