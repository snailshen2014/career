...
public interface BaseMapper {
	   ...
	   @Select("SELECT a.REC_ID RecId,a.REC_ID ACTIVITY_SEQ_ID,b.SUCESSTYPE, "
	   		...
	   		+ "a.LAST_ORDER_CREATE_TIME  LastOrderCreateTime ,b.MATCHINGTYPE matchingType,b.SUCCESS_TYPE_CON_SQL,b.ISHAVERENEWPRODUCT isHaveRenewProduct "
	...
	// --- 提取小沃渠道工单表 ---
		@Select("SELECT REC_ID orderRecId,USER_ID userId FROM PLT_ORDER_INFO_SMALLWO "
				+ "WHERE ACTIVITY_SEQ_ID = #{ActivitySeqId} AND ORDER_STATUS = '5' AND "
				+ "CHANNEL_STATUS  IN('0','2') "
				+ " AND TENANT_ID=#{TenantId}  AND REC_ID  > #{orderRecId}  "
				+ " ORDER BY REC_ID LIMIT 5000")
		public  List<OrderCheckInfo> getSmallwoOrderForAvtivity(@Param("TenantId")String TenantId,
				@Param("ActivitySeqId")int ActivitySeqId,@Param("orderRecId")long orderRecId);	

	// --- 提取玩转流量APP渠道工单表 ---
		@Select("SELECT REC_ID orderRecId,USER_ID userId FROM PLT_ORDER_INFO_PLAY "
				+ "WHERE ACTIVITY_SEQ_ID = #{ActivitySeqId} AND ORDER_STATUS = '5' AND "
				+ "CHANNEL_STATUS  IN('0','2') "
				+ " AND TENANT_ID=#{TenantId}  AND REC_ID  > #{orderRecId}  "
				+ " ORDER BY REC_ID LIMIT 5000")
		public  List<OrderCheckInfo> getPlayOrderForAvtivity(@Param("TenantId")String TenantId,
				@Param("ActivitySeqId")int ActivitySeqId,@Param("orderRecId")long orderRecId);
		// --- 提取新沃视窗渠道工单表 ---
		@Select("SELECT REC_ID orderRecId,USER_ID userId FROM PLT_ORDER_INFO_NEWWSC "
				+ "WHERE ACTIVITY_SEQ_ID = #{ActivitySeqId} AND ORDER_STATUS = '5' AND "
				+ "CHANNEL_STATUS  IN('0','2') "
				+ " AND TENANT_ID=#{TenantId}  AND REC_ID  > #{orderRecId}  "
				+ " ORDER BY REC_ID LIMIT 5000")
		public  List<OrderCheckInfo> getNewWSCForAvtivity(@Param("TenantId")String TenantId,
				@Param("ActivitySeqId")int ActivitySeqId,@Param("orderRecId")long orderRecId);
	   ...
	   // --- 更新序列表 ---
	    ...
	    @Select("SELECT COUNT(1) FROM PLT_SUCCESS_PROCESS_LOG WHERE TENANT_ID=#{tenantId} AND DATE_ID=#{countDateId} AND DATE(PROCESS_TIME)=CURDATE() ")
...
//		public String getChangeTableDateId(@Param("TenantId")String TenantId);
	    ...
	    @Select("SELECT MAX(REC_ID) REC_ID FROM PLT_USER_CHANGE WHERE TENANT_ID=#{TenantId}")
      	public String changeTableMaxRecId(@Param("TenantId")String TenantId);
	    

	@Select("SELECT REC_ID orderRecId,USER_ID userId  FROM ${tableName} "
			+ " WHERE   ACTIVITY_SEQ_ID =${activitySeqId}  AND ORDER_STATUS ='5' AND " + " CHANNEL_STATUS  IN('0','2') "
			+ " AND TENANT_ID=#{tenantId} " + " ORDER BY REC_ID asc")
	public List<OrderCheckInfo> getOrderListForActivityV3(@Param("tenantId") String tenantId,
			@Param("activitySeqId") int activitySeqId, @Param("tableName") String tableName);

	// --- 提取需要判断是否成功的工单（工单生成后）
	@Select("SELECT REC_ID orderRecId,USER_ID userId  FROM ${tableName} "
			+ " WHERE   ACTIVITY_SEQ_ID=${ActivitySeqId}  AND ORDER_STATUS ='5'  AND CHANNEL_STATUS  IN('0','2')  "
			+ " AND TENANT_ID=#{TenantId} AND REC_ID>=${beginRecId} AND REC_ID<=${endRecId}")
	public List<OrderCheckInfo> getRequiredOrderListForSucess(@Param("TenantId") String TenantId,
			@Param("ActivitySeqId") int ActivitySeqId, @Param("tableName") String tableName,
			@Param("beginRecId") int beginRecId, @Param("endRecId") int endRecId);
	
	 @Insert("INSERT INTO SYS_COMMON_CFG(CFG_KEY,CFG_VALUE) VALUES(#{strCfgKey},#{strCfgValue})")
	 public void insertSysCommCfg(@Param("strCfgKey") String strCfgKey,@Param("strCfgValue") String strCfgValue);
	 //根据批次查询该批次下有哪些渠道
	 @Select("SELECT CHANN_ID FROM PLT_ACTIVITY_CHANNEL_DETAIL WHERE TENANT_ID=#{tenantId} AND ACTIVITY_SEQ_ID=${activitySeqId}")
	public List<String> queryChannelIdsBySeqId(@Param("activitySeqId")int activitySeqId, @Param("tenantId")String tenantId);
	
	//查询表里所有的批次
	 @Select("SELECT DISTINCT ACTIVITY_SEQ_ID FROM ${tableName} WHERE TENANT_ID=#{tenantId}")
	public List<Integer> queryAllSeqIds(@Param("tableName")String tableName, @Param("tenantId")String tenantId);
