...
public class MapUtil {
	...
	/**
	 * @Title: convertMapKey
	 * @Description: 替换mapList中的每个map元素的key名称
	 * @return: Boolean
	 * @param mapList
	 * @param keyMap
	 * @return
	 * @throws: 
	 */
	public static Boolean convertMapKey(List<HashMap> mapList,
			HashMap<String,String> keyMap){
		Boolean result = false;
		if(mapList == null || keyMap == null || 
				mapList.size() < 1 || keyMap.size() < 1)
			return result;
		...
		for(HashMap<String,Object> map:mapList){
			...
			while(iterator.hasNext()){
				...
				if(keyMap.get(key) != null){		
					resultMap.put(keyMap.get(key), map.get(key));
				}
				iterator.remove();
			}
