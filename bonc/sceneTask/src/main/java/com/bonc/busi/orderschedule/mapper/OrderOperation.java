...
public class OrderOperation {
	...
	public String updateOrderFinishedStatus(PltActivityInfo at) {
		String s = "'";
		StringBuilder where = new StringBuilder();
	...
	// 优化后代码
	public String selectUpdateRuleByActivity() {
		BEGIN();
		SELECT("REC_ID,ORDER_GEN_RULE,LAST_ORDER_CREATE_TIME,ORDER_UPDATE_RULE");
		FROM(" PLT_ACTIVITY_INFO ");
		WHERE(" (ORDER_GEN_RULE = 0 OR ORDER_GEN_RULE = 1) AND ACTIVITY_STATUS=1 AND TENANT_ID='uni076'");
		String sql = SQL();
		logger.info(" sql:" + sql);
	...
	public String insertOrderInfoHis(OrderAndOrderSMS in) {
		String s = "'";
		BEGIN();
		INSERT_INTO("PLT_ORDER_INFO_HIS");
	...
	public String getInvalidActivitySeqId(String tenantId) {
		String s = "'";
		StringBuilder whereBuilder = new StringBuilder();
		whereBuilder.append(" END_DATE < SYSDATE()");
		whereBuilder.append(" AND ORDER_STATUS !='0'");
		whereBuilder.append("AND TENANT_ID= " + s + tenantId + s);
		BEGIN();
		SELECT("distinct ACTIVITY_SEQ_ID ");
		FROM(ORDER_INFO);
		WHERE(whereBuilder.toString());
		return SQL();
	}
	...
	public String selectUpdateRuleByActivity(ParamMap param) {
		...
		FROM(" PLT_ACTIVITY_INFO ");
		WHERE(" (ORDER_GEN_RULE = 1 OR ORDER_GEN_RULE = 2)  AND ACTIVITY_STATUS = 1 AND TENANT_ID='"
				+ param.getTENANT_ID() + "' AND ACTIVITY_ID = '" + param.getACTIVITY_ID() + "'");
	...
	public String selectActivityForTouch(ParamMap param) {
		...
		SELECT("REC_ID");
		FROM(" PLT_ACTIVITY_INFO ");
		WHERE(" ACTIVITY_STATUS = 1 AND TENANT_ID='" + param.getTENANT_ID() + "' AND ACTIVITY_ID = '"
				+ param.getACTIVITY_ID() + "'");
		ORDER_BY(" LAST_ORDER_CREATE_TIME DESC");
		String sql = SQL();
		logger.info("selectUpdateRuleByActivity sql:" + sql);
		return sql;
