...
public class StatisticServiceImpl implements StatisticService{
		...
		for(HashMap<String, Object> recId:recIds){
			try{
				...
				if(IContants.DX_CHANNEL.equals(recId.get("CHANNEL_ID"))){
					req.put("TABLE_NAME", orderTable.get(recId.get("CHANNEL_ID"))+"_HIS_"+recId.get("MONTH"));
				}else{
					req.put("TABLE_NAME", orderTable.get(recId.get("CHANNEL_ID")));
				}
				
				
				HashMap<String, Object> statistic = mapper.getRecStatistic(req);
				if(null==statistic){
					log.warn("批次统计为空！");
					continue;
				}
	...
	private void initLoginId(HashMap<String, Object> map) {
		...
		sql.append("WHERE o.TENANT_ID='").append(map.get("tenantId")).append("' AND l.TENANT_ID='").append(map.get("tenantId")).append("' AND o.ACTIVITY_SEQ_ID=").append(map.get("activitySeqId")).append(" AND l.ORG_PATH=o.ORG_PATH AND o.WENDING_FLAG IS NULL ");
	...
	public void incrStatistic(HashMap<String, Object> map) {
		...
		for(HashMap<String, Object> order:list){
			...
			try{
				...
				start = System.currentTimeMillis();
				Integer CONTACT_FILTER_NUM = statisticRuleRec(order);
				end = System.currentTimeMillis();
				...
				start = System.currentTimeMillis();
				Integer RULE_NUM = statisticRuleRec(order);
				end = System.currentTimeMillis();
				...
				start = System.currentTimeMillis();
				Integer REMAIN_NUM = statisticRuleRec(order);
				end = System.currentTimeMillis();
